...
(function(dust) {
	...
	Chunk.prototype.section = function(elem, context, bodies, params) {
		...
		/*
		 * Dust's default behavior is to enumerate over the array elem, passing each object in the array to the block. When elem resolves to a value or object instead of an array, Dust sets the
		 * current context to the value and renders the block one time.
		 */
...
(function(dust) {
	...
	function compile(ast, name) {
		...
		return "(function(){dust.register(" + (name ? "\"" + name + "\"" : "null") + "," + dust.compileNode(context, ast) + ");" + compileBlocks(context) + compileBodies(context) + "return body_0;"
				+ "})();";
	}

	function compileBlocks(context) {
		var out = [], blocks = context.blocks;
	...
	function compileBodies(context) {
		var out = [], bodies = context.bodies, blx = context.blocks;
	...
	dust.nodes = {
		...
		reference : function(context, node) {
			return ".reference(" + dust.compileNode(context, node[1]) + ",ctx," + dust.compileNode(context, node[2]) + ")";
		},
		...
		"<" : function(context, node) {
			...
			for ( var i = 1, len = bodies.length; i < len; i++) {
				var param = bodies[i], type = param[1][1];
				if (type === "block") {
		...
		"+" : function(context, node) {
			if (typeof (node[1].text) === "undefined" && typeof (node[4]) === "undefined") {
				return ".block(ctx.getBlock(" + dust.compileNode(context, node[1]) + ",chk, ctx)," + dust.compileNode(context, node[2]) + ", {}," + dust.compileNode(context, node[3]) + ")";
			} else {
				return ".block(ctx.getBlock(" + escape(node[1].text) + ")," + dust.compileNode(context, node[2]) + "," + dust.compileNode(context, node[4]) + "," + dust.compileNode(context, node[3])
						+ ")";
			}
		},

		"@" : function(context, node) {
			return ".helper(" + escape(node[1].text) + "," + dust.compileNode(context, node[2]) + "," + dust.compileNode(context, node[4]) + "," + dust.compileNode(context, node[3]) + ")";
		},
		...
		partial : function(context, node) {
			return ".partial(" + dust.compileNode(context, node[1]) + "," + dust.compileNode(context, node[2]) + "," + dust.compileNode(context, node[3]) + ")";
		},
		...
		filters : function(context, node) {
			...
			return "\"" + context.auto + "\"" + (list.length ? ",[" + list.join(',') + "]" : '');
		},
		...
		path : function(context, node) {
			var current = node[1], keys = node[2], list = [];
	...
	function compileSection(context, node, cmd) {
		return "." + cmd + "(" + dust.compileNode(context, node[1]) + "," + dust.compileNode(context, node[2]) + "," + dust.compileNode(context, node[4]) + "," + dust.compileNode(context, node[3])
				+ ")";
	}

	var escape = (typeof JSON === "undefined") ? function(str) {
...
(function(dust) {
	...
	var parser = (function() {
		...
		function quote(s) {
			/*
			 * ECMA-262, 5th ed., 7.8.4: All characters may appear literally in a string literal except for the closing quote character, backslash, carriage return, line separator, paragraph
			 * separator, and line feed. Any character may appear in the form of an escape sequence.
			 * 
			 * For portability, we also escape escape all control and non-ASCII characters. Note that "\0" and "\v" escape sequences are not used because JSHint does not like the first and IE the
			 * second.
			 */
		...
		var result = {
			/*
			 * Parses the input with a generated parser. If the parsing is successfull, returns a value explicitly or implicitly specified by the grammar from which the parser was generated (see
			 * |PEG.buildParser|). If the parsing is unsuccessful, throws |PEG.parser.SyntaxError| describing the error.
			 */
			parse : function(input, startRule) {
				 ...
				 *  - |result !== null| - |pos.offset === input.length| - |rightmostFailuresExpected| may or may not contain something
				 * 
				 * 2. The parser successfully parsed only a part of the input.
				 *  - |result !== null| - |pos.offset < input.length| - |rightmostFailuresExpected| may or may not contain something
				 * 
				 * 3. The parser did not successfully parse any part of the input.
				 *  - |result === null| - |pos.offset === 0| - |rightmostFailuresExpected| contains at least one failure
				 * 
				 * All code following this comment (including called functions) must handle these states.
				 */
				if (result === null || pos.offset !== input.length) {
					...
					throw new parser.SyntaxError(cleanupExpected(rightmostFailuresExpected), found, offset, errorPosition.line, errorPosition.column);
				}
